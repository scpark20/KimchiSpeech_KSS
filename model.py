...
class ReferenceEncoder(nn.Module):
    def __init__(self, hp):
        super().__init__()
        self.hp = hp
        self.convs = nn.Sequential(nn.Conv2d(1, hp.ref_hidden_channels, kernel_size=3, stride=2, padding=1),
                                   nn.BatchNorm2d(hp.ref_hidden_channels),
                                   nn.ReLU(),
                                   nn.Conv2d(hp.ref_hidden_channels, hp.ref_hidden_channels, kernel_size=3, stride=2, padding=1),
                                   nn.BatchNorm2d(hp.ref_hidden_channels),
                                   nn.ReLU(),
                                   nn.Conv2d(hp.ref_hidden_channels, hp.ref_hidden_channels, kernel_size=3, stride=2, padding=1),
                                   nn.BatchNorm2d(hp.ref_hidden_channels),
                                   ...
                                   nn.BatchNorm2d(hp.ref_hidden_channels),
                                   nn.ReLU())
        self.gru = nn.GRU(hp.ref_hidden_channels*2, hidden_size=hp.ref_hidden_channels//2,
                          batch_first=True, bidirectional=True)
        self.out_linear = nn.Sequential(nn.Linear(hp.ref_hidden_channels, hp.ref_out_channels))
    ...
    def forward(self, x):
        ...
        x = self.convs(x)
        # (batch, time, dim*mel)
        x = x.reshape(-1, x.size(1)*x.size(2), x.size(3)).transpose(1, 2)
        # (2, batch, dim)
        _, x = self.gru(x)
        # (batch, dim)
        x = x.transpose(0, 1).reshape(-1, 2*x.size(2))
        # (batch, dim)
        x = self.out_linear(x)
        return x
...
#         self.convs = nn.Sequential(nn.Conv2d(1, 32, kernel_size=3, stride=2, padding=1),
#                                    nn.BatchNorm2d(32),
#                                    nn.GELU(),
...
#                                    nn.BatchNorm2d(128),
#                                    nn.ReLU())
#         self.gru = nn.GRU(256, 128, batch_first=True, bidirectional=False)
#         self.out_linear = nn.Sequential(nn.Linear(128, ref_channels*8),
#                                         nn.Tanh(),
#                                         nn.Linear(ref_channels*8, ref_channels*2),
#                                         nn.Tanh(),
#                                         nn.Linear(ref_channels*2, ref_channels))
...
class TTSModel(nn.Module):
    ...
    def forward(self, batch, stt_outputs, beta=1.0):
        ...
        stt_params = batch['alignment_params'].detach()
    ...
    def inference(self, cond, alignments=None, mel_length=None, temperature=1.0, speed=1.0, clip=None):
        ...
        if pad_length > 0:
            y = y[:, :, :-pad_length]
            alignments = alignments[:, :, :-pad_length]
        return y, alignments, time_dict
...
class Model(nn.Module):
    ...
    def forward(self, batch):
        ...
        tts_outputs = self.tts(batch, self.beta)
        
        return tts_outputs
        
    def inference(self, cond, z_sample=None, alignments=None, mel_length=None, temperature=1.0, speed=1.0, clip=None):
